"""
PV power model taken from renewables ninjas modified for our purpose

main changes:
    - calculate angles changed to sun_elevation(==sun_alt) and sun_azimuth
    - remove tracking
    - if  tilt is not given, it will be calculated by optimal_tilt(lat)
...
class PVPanel(object):
    ...
    def __init__ (self, panel_aperture=1.0, panel_ref_efficiency=1.0,
                  c_temp_amb=1, c_temp_irrad=35, **kwargs):
...
def run_plant_model (ghi, dhi, dni, coords, capacity=1, tilt=None, tamb=None, azim=180,
                     technology='csi', system_loss=0.10, sun_azimuth=None, sun_elevation=None,
    ...
    irrad = trigon.poa_irradiance((ghi - dhi), dhi,dni, coords,
    ...
    output = panel.panel_power(direct=irrad.direct,
